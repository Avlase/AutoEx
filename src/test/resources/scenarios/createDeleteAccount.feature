Feature: Create/Delete account
  ...
  Scenario Outline: Create an account via API call
   ...
   | User03 | 006user@mailinator.com |   Mr    |      1    |    3        | 1973       | QAZ123wsx | Threeuser | Lthree   | Address 3  | Address 23 | Unites States | NY    | NY   | 100001  | 3223224424    |
  ...
  Scenario Outline: Delete account via API if account doesn't exists
    When I send an Api call to delete the account with: '<Email>', '<Password>'
    Then response body contains 'Account not found!' message
    Examples:
      | Email                     | Password  |
